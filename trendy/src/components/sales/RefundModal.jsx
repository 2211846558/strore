...
const RefundModal = ({ isOpen, onClose, item, onConfirm }) => {
      ...
      <div className="sales-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="sales-policy-box">
          ...
          <div>
            ...
            <p className="sales-policy-text">يمكن استرداد المنتجات خلال 7 أيام من تاريخ الشراء</p>
          </div>
        </div>
        <div className="sales-refund-details">
          ...
          <p className="sales-refund-meta">اللون: {item.color} | المقاس: {item.size}</p>
          <p className="sales-refund-amount">- {item.price * item.quantity} د.ل</p>
        </div>
        <div className="sales-modal-footer">
          <button type="button" className="sales-btn-primary" onClick={() => { onConfirm(); onClose(); }}>تأكيد الاسترداد</button>
